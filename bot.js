...
const commandsList = {
    ping: '[Tetsowa komenda] Odpowiada Pong!',
    ai: 'Pobiera odpowiedź AI. Użycie: !ai <twoje pytanie>',
    help: 'Wyświetla listę dostępnych komend.',
    setgender: 'Ustaw swoją płeć dla poprawnego tłumaczenia na język polski. Użycie: !setgender <f/m>',
    setlang: 'Ustaw preferowany język (polski/angielski). Użycie: !setlang <pl/en>',
};
...
const commandsListEn = {
    ...
    setgender: 'Set your gender for proper Polish translation. Usage: !setgender <f/m>',
...
discordClient.on('messageCreate', async message => {
    ...
    const command = message.content.slice(prefix.length).split(/\s+/)[0].toLowerCase();
    const fullContent = message.content.slice(prefix.length + command.length + 1);
    ...
    switch (command) {
        ...
        case 'ai':
            try {
                ...
                const englishQuery = await translateToEnglish(fullContent.trim());

                const chatCompletion = await groqClient.chat.completions.create({
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an emotional counselor for people with problems. You need to help them with their problems when they describe them. Be empathetic, supportive, and provide practical advice when appropriate.'
                        },
                        { 
                            role: 'user', 
                ...
                const polishResponse = await translateToPolish(aiResponse, isFemale);
        ...
        case 'help':
            ...
            const helpMessage = Object.entries(helpList)
                ...
                .join('\n');
            const helpTitle = userLang === 'pl' ? 'Dostępne komendy:' : 'Available commands:';
